...
class UpdateEngine:
    ...
    async def _debounced_refresh(self, _now=None) -> None:
        ...
        entity_id = self._pending_entity
        source_changed = self._pending_source_changed
        processed = await self._async_refresh(
            "source_event", entity_id, source_changed=source_changed
        )

        # A mapped source may change while the live refresh or its listeners are
        # still running. Never discard that newer event: schedule one more
        # coalesced pass so House/Solar/Grid/Battery converge on the latest HA
        # states instead of waiting for the next device update. This also covers
        # the narrow rate-limit window below.
        if processed and generation == self._pending_generation:
            self._pending_entity = None
            self._pending_source_changed = None
        if (not processed or generation != self._pending_generation) and self._unsub_debounce is None:
            self._unsub_debounce = async_call_later(
                self.hass, self.DEBOUNCE_SECONDS, self._debounced_refresh
            )
    ...
    async def _async_refresh(
        self, reason: str, entity_id: str | None, *, source_changed: datetime | None = None
    ) -> bool:
